  ...
  return (
    <div style={{ height: "100vh", display: "flex" }}>
      <div style={{ width: "20%", padding: "10px", background: "#f4f4f4" }}>
        <h3>Actions</h3>
        {["Send Connection Request", "If Accepted", "If Rejected", "View Profile", "Like Post", "Send Message", "Follow Lead", "End Campaign"].map(
          (action) => (
            <button key={action} onClick={() => addNode(action)} style={{ display: "block", marginBottom: "5px" }}>
              {action}
            </button>
          )
        )}
